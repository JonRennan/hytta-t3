...
import {getBookingById, getCabinById, getPublicOrPrivateCabins,} from "~/server/queries";
import {type Cabin, CABIN_READ_ACCESS, CABIN_WRITE_ACCESS} from "~/types";
...
export async function getMyCabins(): Promise<Cabin[]> {
    ...
    const returnCabins = privateCabins.filter((cabin) =>
