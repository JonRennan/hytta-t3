...
import {differenceInCalendarDays, isBefore, isPast, isSameDay, isSameMonth, isWithinInterval,} from "date-fns";
import {cn} from "~/lib/utils";
import {type Booking, today} from "~/types";
...
export function getCellStyles(
    ...
    if (isSameDay(cellDay, today)) {
        todayStyle = "inset-ring-4 inset-ring-surface-on_variant";
    }
    if (booking) {
        return cn(
            "pointer-events-none",
...
export function getSelectedSpanStyles(
    ...
    if (booking) {
        const className = "h-2 w-full absolute top-0 left-0";
        if (booking.bookingType === "AirBnB") {
