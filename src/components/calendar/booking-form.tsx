...
import {getBookingIntervals, selectionContainsBooking,} from "~/lib/calendar/utils";

import {cn} from "~/lib/utils";
import {createBooking, editBooking} from "~/server/actions";
import {type Booking, bookingTypeEnum, formatDateString, formatDbDate, today,} from "~/types";
...
export function BookingForm({
    ...
    const bookingFormSchema = z
        ...
        .refine(
            (data) => {
                const selection = selectionContainsBooking(
                    data.fromDate,
                    data.toDate,
                    bookings,
                );
    ...
    const form = useForm<z.infer<typeof bookingFormSchema>>({
        ...
        defaultValues: {
            bookingType: prevBookingType ?? "Private",
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    ...
                    render={({field}) => (
                        <FormItem className="flex flex-wrap items-center">
                            <div className="flex w-full items-center gap-4">
                                <FormLabel className="w-1/8">Fra</FormLabel>
                                <Popover>
                                    <PopoverTrigger asChild>
                                        <FormControl>
                                            <Button
                                                variant={"secondary"}
                                                className={cn(
                                                    "w-7/8 pl-3 text-left font-normal",
                ...
                <FormField
                    ...
                    render={({field}) => (
                        <FormItem className="flex flex-wrap items-center">
                            <div className="flex w-full items-center gap-4">
                                <FormLabel className="w-1/8">Til</FormLabel>
                                <Popover>
                                    <PopoverTrigger asChild>
                                        <FormControl>
                                            <Button
